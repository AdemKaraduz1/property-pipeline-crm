...
export async function PATCH(
  ...
  const settings = parseDealAnalyzerSettings(asRecord(body).settings);

  if (!settings) {
    return NextResponse.json(
      { success: false, message: "Invalid deal analyzer settings." },
      { status: 400 },
    );
  }
  ...
  if (updateError) {
    return NextResponse.json(
      {
        success: false,
        message: "Could not save deal analyzer settings.",
