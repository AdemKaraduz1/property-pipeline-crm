...
import { parseNegotiationRounds } from "@/lib/negotiation";
...
export async function PATCH(
  ...
  if (!body || typeof body !== "object") {
    return NextResponse.json(
      { success: false, message: "Invalid negotiation log." },
      { status: 400 },
    );
  }

  const rounds = parseNegotiationRounds(body);
  ...
  if (updateError) {
    return NextResponse.json(
      {
        success: false,
        message: "Could not save negotiation log.",
